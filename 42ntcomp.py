...
#for defline, seq in mcb185.read_fasta(sys.argv[1]):
    ...
    #for nt in seq:
        ...
        #elif nt == 'T': T += 1
        #else: N += 1
    #print(name, A/len(seq), G/len(seq), T/len(seq), C/len(seq))
...
#for defline, seq in mcb185.read_fasta(sys.argv[1]):
    ...
    #name = defwords[0]
    #nts = 'ACGTN'
    #counts = [0] * len(nts)         # shortcut for assigning zeroes
    #for nt in seq:
        #idx = nts.find(nt)          # if the nt is found, its index is returned
        #counts[idx] += 1
    #print(name, end='  ')
    #for n in counts: print(n/len(seq), end='   ')
    #print()
...
#for defline, seq in mcb185.read_fasta(sys.argv[1]):
    ...
    #counts = []
    #for nt in seq: 
        #if nt not in nts:       # finds new letters and adds to nts
            #nts.append(nt)
            #counts.append(0)
        #idx = nts.index(nt)
        #counts[idx] += 1
